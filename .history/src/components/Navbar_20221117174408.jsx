    ...
    <nav className={ `flex w-full items-center justify-between py-6  `} >
        ...
        <ul className=' items-center justify-center  gap-14    sm:flex  hidden'>
            {navLinks.map((link)=>{
                return (
                    <li key={link.id}>
                        <a className=' text-slate-100 dark:text-black hover:text-cyan-500 transition duration-300' href={`#${link.id}`}>{link.title}</a>
                    </li>
                )
            })}
            <button onClick={()=>setdark()} className=' text-[30px] text-white'>  <BsFillBrightnessHighFill /></button>
        </ul>
